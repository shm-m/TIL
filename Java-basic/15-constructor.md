...
**생성자**란 클래스의 인스턴스가 생성될 떄 호출되는 **인스턴스 초기화 메서드**로 해당 클래스와 이름이 똑같은 메소드이다.

```java
Sample s = new Samle();
```

기존의 인스턴스를 초기화하는 코드에서 사용된 `Sample()`이 바로 생성자를 통해 작성되는 형태임.

생성자 역시 메서드이므로 클래스 내부에서 선언되며, 주의할 점은 **모든 생성자는 리턴값을 가지지 않는다는 것이다.**
...
생성자를 별도로 만들지 않아도 자바는 자동으로 컴파일러를 통해 **기본 생성자**를 만들어준다. 기본생성자는 위와 같은 형식을 가지며 <u>실제로는 클래스에서 보이지 않는다.</u>

이때, 기본 생성자는 참조형 필드는 null로 기본형 필드는 0으로 초기화해준다. 기본 생성자는 파라미터가 존재하지 않는다.
...
생성자는 반드시 기본생성자만을 사용할 필요가 없으며, 상황에 따라서는 재정의가 가능하다. 또한 **오버로딩**을 통해 여러 개를 생성할 수 있다.

단 주의할 점은 **사용자가 직접 생성자를 만들 경우, 기본생성자는 더 이상 사용할 수 없다.** 따라서 기본 생성자를 사용하고 싶으면 사용자가 직접 작성해서 명시해줘야 한다.
...
생성자도 메서드처럼 **파라미터**를 선언하여 호출 시 값을 넘겨받아 인스턴스 초기화 작업에 쓸 수 있다. <u>이는 각각의 인스턴스마다 인스턴스변수값을 달리해야하는 클래스에 유용하다.</u>

생성자에 파라미터를 두면 인스턴스 생성 이후 인스턴스변수들을 별도로 초기화할 필요가 없어서 편리함.
...
생성자는 인스턴스 생성 시 반드시 호출될 수 밖에 없음. 앞서 말했듯이 이전까지 클래스의 인스턴스를 생성 시 다음과 같은 코드를 작성했는데,

```java
Sample s = new Sample(인수 목록);
```

이때 사용되는 `Sample()`이 생성자를 사용한 것이다. 위의 코드는 파라미터가 존재하는 생성자로 인스턴스를 생성 시에 호출된 생성자의 형태이다.

객체는 연산자 `new`에 의해서 생성되며, 이때 호출된 생성자가 인스턴스변수들을 초기화한다. 그리고 연산자 `new`의 결과로 인스턴스의 주소가 반환되어 참조변수 s에 저장되는 것이다.
...
파라미터를 특정 인스턴스로 둘 경우, 해당 인스턴스변수들이 생성자를 통해 복사된다. 이는 서로 독립적인 메모리공간을 확보하므로(기본형 데이터타입이기 때문에) 생성 이후 값들이 변경되어도 서로 영향을 주지 않는다.
